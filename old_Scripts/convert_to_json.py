...
def colmap_to_json(
    ...
    Args:
        output_dir: Path to the reconstruction directory, e.g. "sparse/0"
    ...
    cam_id_to_camera = read_cameras_binary(output_dir + "colmap/sparse/0/cameras.bin")
    im_id_to_image = read_images_binary(output_dir + "colmap/sparse/0/images.bin")
    ...
    with open(output_dir + "colmap/transforms.json", "w", encoding="utf-8") as f:
        json.dump(out, f, indent=4)

    return len(frames)

def main():
    parser = argparse.ArgumentParser(description='Script with an output directory option')
    parser.add_argument('--output_dir', dest='output_dir', required=True, help='Specify the output directory')

    args = parser.parse_args()

    output_dir = args.output_dir

    suc = colmap_to_json(output_dir)
    print(f'succesfully converted {suc} frames')
    # You can perform additional tasks here using the output directory
